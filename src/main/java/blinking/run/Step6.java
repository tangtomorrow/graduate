...
public class Step6 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Table finalTable = gets().stream()
                ...
                .map(station -> {
                    ...
                    // 增加27个列
                    try {
                        Table uhiiTable = Table.read().csv("data/x/uhii/" + station.getStation().getName() + ".csv");
        ...
        finalTable.write().csv("data/x/total_stations_uhii.csv");
    }
    ...
    private static void getUHII(Table type53) {
        gets().stream()
                .filter(stationDetail2 -> stationDetail2.getStation().getType12() != 53
                        && stationDetail2.getStation().getType2() != 2)
                .map(stationDetail2 -> {
                    Table t12 = stationDetail2.getTable().groupBy("hour").getSubTables()
                            .stream()
            ...
            try {
                table.write().csv("data/x/uhii/" + table.name() + ".csv");
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
    ...
    private static Table type53(Table data) throws IOException {
        Table t = data.selectWhere(column("type12").isEqualTo(53)).groupBy("hour")
                .getSubTables()
                .stream()
                .map(TemporaryView::asTable)
                .sorted(Comparator.comparingInt(o -> Integer.parseInt(o.name())))
